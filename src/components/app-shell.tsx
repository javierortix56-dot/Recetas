...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative w-full h-full min-h-screen overflow-hidden">
      ...
      <div className={cn("tab-panel absolute inset-0 overflow-y-auto overscroll-none pb-40 px-4 pt-16", (activeTab === 'inicio' && isCoreTab) ? 'visible' : 'hidden')}>
        <InicioTab />
      </div>
      <div className={cn("tab-panel absolute inset-0 overflow-y-auto overscroll-none pb-40 px-4 pt-16", (activeTab === 'planificacion' && isCoreTab) ? 'visible' : 'hidden')}>
        <PlanificacionTab />
      </div>
      <div className={cn("tab-panel absolute inset-0 overflow-y-auto overscroll-none pb-40 px-4 pt-16", (activeTab === 'recetas' && isCoreTab) ? 'visible' : 'hidden')}>
        <RecetasTab />
      </div>
      <div className={cn("tab-panel absolute inset-0 overflow-y-auto overscroll-none pb-40 px-4 pt-16", (activeTab === 'stock' && isCoreTab) ? 'visible' : 'hidden')}>
        <StockTab />
      </div>
      <div className={cn("tab-panel absolute inset-0 overflow-y-auto overscroll-none pb-40 px-4 pt-16", (activeTab === 'compras' && isCoreTab) ? 'visible' : 'hidden')}>
        <ComprasTab />
      </div>
      <div className={cn("tab-panel absolute inset-0 overflow-y-auto overscroll-none pb-40 px-4 pt-16", (activeTab === 'macros' && isCoreTab) ? 'visible' : 'hidden')}>
        <MacrosTab />
      </div>
