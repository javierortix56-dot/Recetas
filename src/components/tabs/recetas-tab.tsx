    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 pb-32">
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <div className="flex gap-2">
            {isSelectionMode ? (
              ...
              <>
                <Button size="icon" variant="ghost" onClick={() => router.push("/recetas/nueva")} className="rounded-full bg-primary text-white shadow-sm">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="rounded-2xl">
                    ...
                    <DropdownMenuItem onClick={() => setIsImportOpen(true)} className="gap-3 font-medium">
        ...
        {!isSelectionMode && (
          <>
            <div className="relative">
              ...
              <Input placeholder="Buscar recetas..." className="pl-10 h-11 bg-white rounded-xl font-medium shadow-sm border border-border" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>

            <div className="flex flex-wrap gap-2 pb-1 items-center">
              {CATEGORIES.map((cat) => (
                <Badge key={cat} variant={activeCategory === cat ? "default" : "secondary"} className={cn("px-3.5 py-1.5 rounded-full cursor-pointer whitespace-nowrap text-xs font-medium snap-start transition-all", activeCategory === cat ? "bg-primary text-white shadow-sm" : "bg-primary-suave text-primary border-none")} onClick={() => setActiveCategory(cat)}>{cat}</Badge>
              ))}
              <Button variant="ghost" size="sm" onClick={() => setIsTagsExpanded(!isTagsExpanded)} className={cn("rounded-full h-8 px-3 shrink-0 font-medium text-xs gap-1", isTagsExpanded ? "bg-primary text-white" : "bg-primary-suave text-primary")}>
                ...
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                  <div className="flex flex-wrap gap-2 pt-2 px-1">
                    {allTagsSorted.slice(0, 20).map((tag) => (
                      <Badge key={tag} onClick={() => setActiveTag(activeTag === tag ? null : tag)} className={cn("px-2.5 py-1 rounded-lg cursor-pointer text-xs font-medium transition-all border-none", activeTag === tag ? "bg-primary text-white" : "bg-muted text-muted-foreground")}>#{tag}</Badge>
                    ))}
                  </div>
                </motion.div>
              )}
      ...
      {filteredRecipes.length > 0 ? (
        ...
        <div className="py-24 text-center">
          <p className="font-medium text-muted-foreground text-sm">No encontramos recetas</p>
        </div>
      )}
...
function RecipeListItem({ recipe, index, isSelectionMode, isSelected, onToggleSelection }: any) {
  ...
  return (
    ...
    >
      <Card onClick={handleClick} className={cn("overflow-hidden border border-border shadow-recipe hover:shadow-card-hover active:scale-[0.97] transition-all rounded-2xl h-full flex flex-col relative group", isSelected ? "ring-2 ring-primary" : "bg-white")}>
        {isSelectionMode && <div className={cn("absolute top-3 right-3 z-20 h-7 w-7 rounded-full flex items-center justify-center border", isSelected ? "bg-primary border-primary text-white" : "bg-white/80 border-primary/20")}>{isSelected && <Check className="h-4 w-4 stroke-[3]" />}</div>}
        <div className="relative h-24 w-full pointer-events-none bg-muted">
          ...
          <div className="absolute bottom-2 left-2 right-2">
            <h3 className="font-semibold text-xs leading-tight text-white drop-shadow-sm line-clamp-2">{recipe.nombre}</h3>
            <span className="flex items-center gap-0.5 text-[11px] font-medium text-white/80 mt-0.5">
              <Flame className="h-2.5 w-2.5" /> {recipe.macros?.calorias || 0}
            </span>
          </div>
          <Badge className="absolute top-1.5 left-1.5 bg-white/90 backdrop-blur-sm text-[10px] font-medium text-primary border-none h-5 px-1.5 shadow-sm">
            {primaryCategory}
          </Badge>
        </div>
