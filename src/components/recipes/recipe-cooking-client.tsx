...
import { X, Timer, ChevronLeft } from "lucide-react"
...
export function RecipeCookingClient({ recipeId }: { recipeId: string }) {
  ...
  if (isLoading || !receta) return <div className="h-screen flex items-center justify-center font-bold">Cargando modo cocina...</div>

  const step = receta.pasos?.[currentStep]
  const progress = ((currentStep + 1) / (receta.pasos?.length || 1)) * 100
    ...
    <div className="fixed inset-0 bg-white z-[100] flex flex-col animate-in slide-in-from-bottom duration-500 overflow-hidden">
      <div className="bg-primary text-white pt-10 pb-4 px-6 relative">
        <div className="flex justify-between items-center mb-4">
          <span className="text-xs font-black uppercase tracking-widest">{receta.nombre} — {currentStep + 1}/{receta.pasos?.length || 0}</span>
      ...
      <div className="p-6 grid grid-cols-2 gap-4 bg-white border-t border-border safe-area-pb max-w-lg mx-auto w-full">
        ...
        <Button className="h-14 rounded-2xl bg-primary text-white font-black uppercase text-xs" onClick={() => currentStep < (receta.pasos?.length || 0) - 1 ? (setCurrentStep(currentStep + 1), setTimeLeft(null), setIsTimerRunning(false)) : router.back()}>
          {currentStep < (receta.pasos?.length || 0) - 1 ? "Siguiente" : "Finalizar 🎉"}
        </Button>
      </div>
    </div>
  )
